...
__global__ void SM4_dec_blocks(int n_block, const u1 *in, u1 *out, const u4 *rkey)
{
 ...
 // }
	...
	for (int i = index; i < n_block; i += stride) {
		...
		//__syncthreads();
		SM4_dec_block(sm1 + 16 * cur_idx, out + 16 * i, rkey);
		/*for (int j = 0;j < SM4_BLOCK_SIZE;j++) {
			out[16 * i + j] = sm2[cur_idx * 16 + j];
		}*/
	}
...
{
	u4* rkey;
	cudaMalloc(&rkey, sizeof(u4) * (SM4_RND_KEY_SIZE / sizeof(u4)));
	...
	cudaMalloc((void**)&d_p, sizeof(u1) * 16 * n_block);
	cudaMalloc((void**)&d_c, sizeof(u1) * 16 * n_block);
	for (int i = 0; i < nStreams; i++)
	{
		//cudaMemcpyAsync(d_p, plain + i * sizeof(u1) * 16 * n_block / nStreams, sizeof(u1) * 16 * n_block / nStreams, cudaMemcpyHostToDevice, streams[i]);
		cudaMemcpyAsync(d_p,
			plain + i * sizeof(u1) * 16 * n_block / nStreams,
			sizeof(u1) * 16 * n_block / nStreams,
			cudaMemcpyHostToDevice,
			streams[i]);
		cudaMemcpyAsync(d_c,
			cipher + i * sizeof(u1) * 16 * n_block / nStreams,
			sizeof(u1) * 16 * n_block / nStreams,
			cudaMemcpyHostToDevice,
			streams[i]);
	}
	for (int i = 0; i < nStreams; i++)
	{
		SM4_enc_blocks << <numBlocks, blockSize, 128 * 16 * 32, streams[i] >> > (n_block, d_p, d_c, rkey);
	}
...
void gpu_sm4_decrypt(uint8_t *plain, const uint8_t *key, const uint8_t *cipher, int n_block)
{
  ...
  for (int i = 0; i < nStreams; i++)
  {
	  SM4_dec_blocks << <numBlocks, blockSize, 128 * 16 * 32, streams[i] >> > (n_block, d_p, d_c, rkey);
  }
  for (int i = 0; i < nStreams; i++)
  {
	  cudaStreamDestroy(streams[i]);
...
void benchmark_sm4_encrypt( uint8_t *p, const uint8_t *key, uint8_t *c, uint8_t* d_p, uint8_t* d_key, uint8_t* d_c, int n_block)
{
	int turns = 100;
	float total_time;
	for (int i = 0; i < turns; i++)
	{
		...
		gpu_sm4_encrypt(p, d_key, c, n_block);
		cudaEventRecord(sp, 0);
		cudaEventSynchronize(sp);
		cudaEventElapsedTime(&elapsedTime, ss, sp);
		total_time += elapsedTime / 1000.0;
		cudaEventDestroy(ss);
		cudaEventDestroy(sp);
	}
	cudaDeviceSynchronize();
	double speed = (double)(16 * n_block) / (1024 * 1024 * total_time / 100);
	printf("SM4_encrypt>>> blocks: %d, time: %f s, speed: %f MB/s\n", n_block, total_time / 100, speed);
}

void benchmark_sm4_decrypt(uint8_t *p, const uint8_t *key, const uint8_t *c, uint8_t* d_p, uint8_t* d_key, uint8_t* d_c, int n_block)
{
	int turns = 100;
	float total_time;
  for(int i=0; i<turns; i++)
  {
	  ...
	  gpu_sm4_decrypt(p, d_key, c, n_block);
	  cudaEventRecord(sp, 0);
	  cudaEventSynchronize(sp);
	  cudaEventElapsedTime(&elapsedTime, ss, sp);
	  total_time += elapsedTime / 1000.0;
	  ...
	  //gpu_sm4_decrypt(d_p, d_key, d_c, n_block);
    //gpu_sm4_decrypt(d_p,d_key,d_c,n_block);
  }
  cudaDeviceSynchronize();
	double speed =(double) (16*n_block)/(1024*1024* total_time / 100);
